...
namespace kobuki {
    ...
    class WalkerController: public yocs::Controller {
        public: WalkerController(ros::NodeHandle & nh_priv, std::string & name): Controller(),
            nh_priv_(nh_priv),
            name_(name),
            ...
            turning_(false),
            turning_direction_(1) {};~WalkerController() {};
             ...
             */
            bool init() {
                enable_controller_subscriber_ = nh_priv_.subscribe("enable", 10, & WalkerController::enableCB, this);
                disable_controller_subscriber_ = nh_priv_.subscribe("disable", 10, & WalkerController::disableCB, this);
                bumper_event_subscriber_ = nh_priv_.subscribe("events/bumper", 10, & WalkerController::bumperEventCB, this);
                cliff_event_subscriber_ = nh_priv_.subscribe("events/cliff", 10, & WalkerController::cliffEventCB, this);
                wheel_drop_event_subscriber_ = nh_priv_.subscribe("events/wheel_drop", 10, & WalkerController::wheelDropEventCB, this);
                ...
                ROS_INFO_STREAM("Velocity parameters: linear velocity = " << vel_lin_ << ", angular velocity = " << vel_ang_ << " [" << name_ << "]");
        ...
        private:
            enum bumper_locations { LEFT, CENTER, RIGHT };
            ...
            cliff_event_subscriber_,
            wheel_drop_event_subscriber_;
            /// Publishers
            ...
            // last touched bumper
            bumper_locations last_touched_bumper;
        ...
        void WalkerController::spin() {
            if (this->getState()) // check, if the controller is active
            {
                ...
                if (change_direction_) {
                    ...
                    ROS_INFO_STREAM("WORST");
                    ROS_INFO_STREAM(((double) std::rand() / (double) RAND_MAX));
                    ROS_INFO_STREAM((M_PI / vel_ang_));
                    ...
                    if(last_touched_bumper == LEFT) {
                        turning_direction_ = -1; // go right?
                    } else if(last_touched_bumper == RIGHT) {
                        turning_direction_ = 1; // go left?
                    }


                    turning_start_ = ros::Time::now();
                    turning_ = true;
                    ROS_INFO_STREAM("Will rotate " << turning_direction_ * turning_duration_.toSec() * vel_ang_ / M_PI * 180 << " degrees. [" << name_ << "]");
                }
